...
async function githubPostJson(path, token, body) {
...
async function createGithubIssueFromRun(runId) {
  const run = db.prepare(`
    SELECT ar.*,c.repository,c.github_token_enc,p.name project_name
    FROM agent_runs ar
    JOIN agent_configs c ON c.project_id=ar.project_id
    JOIN projects p ON p.id=ar.project_id
    WHERE ar.id=?
  `).get(runId);
  if (!run) throw new Error('Agent run not found.');
  const repository = normalizeRepository(run.repository);
  const token = tokenFor(run);
  if (!repository) throw new Error('GitHub repository is not configured.');
  if (!token) throw new Error('A GitHub token with Issues write permission is required to create an issue.');

  const findings = db.prepare('SELECT * FROM agent_findings WHERE agent_run_id=? ORDER BY CASE severity WHEN \'critical\' THEN 1 WHEN \'high\' THEN 2 WHEN \'medium\' THEN 3 ELSE 4 END,id LIMIT 80').all(runId);
  const suggestions = db.prepare('SELECT * FROM agent_test_suggestions WHERE agent_run_id=? ORDER BY id LIMIT 30').all(runId);
  const lines = [
    `## QADeck Agent report #${run.id}`,
    '',
    ...
    '',
    '### Summary',
    run.summary || `Reviewed ${run.files_scanned} files and found ${run.findings_count} review item(s).`,
    '',
    '### Findings'
  ].filter(Boolean);
  if (!findings.length) lines.push('- No static findings.');
  for (const item of findings) {
    const where = item.file_path ? ` — \`${item.file_path}${item.line_number ? ':'+item.line_number : ''}\`` : '';
    lines.push(`- **${String(item.severity).toUpperCase()} · ${item.title}**${where}`);
    if (item.recommendation) lines.push(`  - Suggested fix: ${item.recommendation}`);
  }
  lines.push('', '### Recommended tests');
  if (!suggestions.length) lines.push('- No additional tests were suggested.');
  for (const item of suggestions) lines.push(`- **${item.title}** (${item.test_type}, ${item.priority}) — ${item.rationale || ''}`);
  lines.push('', '> Generated by QADeck. Review findings in context before changing production code.');

  return githubPostJson(`/repos/${repository}/issues`, token, {
    title:`QADeck: ${run.findings_count} finding(s) from Agent run #${run.id}`,
    body:lines.join('\n')
  });
}
