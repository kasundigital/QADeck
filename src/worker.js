...
const enqueueDueSchedules = db.transaction(() => {
  ...
  `).all();
  const insertRun = db.prepare(`INSERT INTO test_runs (project_id,status,queued_at,run_type) VALUES (?,'queued',CURRENT_TIMESTAMP,'crawl')`);
...
const claimNextRun = db.transaction(() => {
  const run = db.prepare(`
    ...
    FROM test_runs WHERE status='queued' ORDER BY id ASC LIMIT 1
  `).get();
  if (!run) return null;
  ...
  if (run.started_at) {
    ...
    db.prepare(`UPDATE test_runs SET pages_scanned=0, issues_count=0, clean_pages=0, trace_path=NULL, video_path=NULL, ai_summary=NULL, started_at=NULL, completed_at=NULL WHERE id=?`).run(run.id);
  }

  const claimed = db.prepare(`
    UPDATE test_runs SET status='running', started_at=CURRENT_TIMESTAMP,
      heartbeat_at=CURRENT_TIMESTAMP, worker_id=?, current_url=NULL, error_message=NULL
  ...
  if (!project) {
    db.prepare(`UPDATE test_runs SET status='failed',completed_at=CURRENT_TIMESTAMP,error_message='Project no longer exists.',worker_id=NULL,heartbeat_at=NULL WHERE id=?`).run(run.id);
    return null;
  }
  return { runId: run.id, project, runType: run.run_type || 'crawl', scenarioId: run.scenario_id || null };
});

async function finishPostProcessing(runId) {
  await summarizeRun(runId).catch(() => null);
  await notifyRun(runId).catch((error) => console.warn('[QADeck notifier]', error.message));
}
...
async function loop() {
  ...
  while (!stopping) {
    ...
    try {
      ...
      db.prepare(`UPDATE test_runs SET status='failed',completed_at=CURRENT_TIMESTAMP,error_message=?,worker_id=NULL,heartbeat_at=NULL,current_url=NULL WHERE id=?`).run(error.message || String(error), job.runId);
    }
    await finishPostProcessing(job.runId);
  }
...
loop().catch((error) => { console.error('[QADeck worker] Fatal worker error:', error); process.exitCode = 1; });
